...
'''File này dùng để train model Lenet'''
...
model.summary()
plot_model(model, to_file='model_lenet.png', show_shapes=True,
    show_layer_names=True,)

early_stopping_patience = 5 # Cho việc dừng sớm nếu train quá 7 epoch mà ko đạt KQ tốt hơn
...
my_callbacks = [
    ...
    tf.keras.callbacks.ModelCheckpoint(filepath='model/' + str(datetime_object) + '/rice_model_Lenet_{epoch:02d}.h5',
                                       save_freq='epoch',
                                       monitor='val_loss',
                                       mode='min',
                                       save_best_only=True,
                                       period=3), # Lưu mỗi 5 epoch nên KQ tốt hơn, sợ cúp điện thì nó ko lưu
...
plt.title('Training and Validation Loss Lenet')
plt.savefig('model\{}\Training and Validation Loss Lenet.png'.format(datetime_object))
plt.show()
