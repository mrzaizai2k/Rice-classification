...
'''File này dùng để train model với model Resnet 50'''
...
# plot_model(model, show_layer_names=True, show_shapes =True, to_file='model_resnet.png', dpi=350)

early_stopping_patience = 3 # Cho việc dừng sớm nếu train quá 7 epoch mà ko đạt KQ tốt hơn
...
my_callbacks = [
    ...
    tf.keras.callbacks.ModelCheckpoint(filepath='model/' + str(datetime_object) + '/rice_model_resnet_{epoch:02d}.h5',
                                       save_freq='epoch',
                                       monitor='val_loss',
                                       mode='min',
                                       save_best_only=True,
                                       period = 2), # Lưu mỗi 5 epoch nên KQ tốt hơn, sợ cúp điện thì nó ko lưu
...
plt.savefig('model\{}\Training and Validation Loss Resnet.png'.format(datetime_object))
plt.show()
